...
import type { PlanTier, PricingResponse, ComputedSubscriptionStatus } from "@/lib/types";
...
interface PlanSelectorProps {
    ...
    pendingPlan?: string;
    pendingStatus?: ComputedSubscriptionStatus;
}
                ...
                return (
                    <button
                        ...
                        className={cn(
                            ...
                            (isCurrent || isPending) && "cursor-not-allowed opacity-60 grayscale-[0.5]"
                        )}
                    >
                        {isCurrent && (
                        ...
                        {isPending && (
                            <div className={cn(
                                "absolute right-4 top-4 flex items-center gap-1.5 rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white",
                                pendingStatus === "under_review" ? "bg-sky-600" : "bg-amber-600"
                            )}>
                                <Check className="h-3 w-3" />
                                {pendingStatus === "under_review" ? "Under Review" : "Pending Verification"}
                            </div>
                        )}
