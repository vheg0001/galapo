...
export default function PlanLimitIndicator({ used, total, className }: PlanLimitIndicatorProps) {
    ...
    return (
        <div className={cn("rounded-2xl border border-border/50 bg-card p-6 shadow-sm", className)}>
            <div className="mb-4 flex items-center justify-between">
                <div>
                    <h3 className="text-sm font-black uppercase tracking-widest text-muted-foreground/60">Deal Slot Usage</h3>
                    <p className="text-2xl font-black tracking-tight text-foreground">
                        {used} <span className="text-muted-foreground/30">/</span> {total}
                    </p>
                </div>
                <div className={cn(
            ...
            <div className="mt-4 flex items-center justify-between">
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                    {isAtLimit ? "Limit reached" : `${total - used} slots remaining`}
                </p>
                {isAtLimit && (
                    <Link
