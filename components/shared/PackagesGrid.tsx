...
export default function PackagesGrid({ packages }: PackagesGridProps) {
    ...
    const gridCols = count >= 4 ? "lg:grid-cols-4" : count === 3 ? "lg:grid-cols-3" : count === 2 ? "md:grid-cols-2" : "max-w-md mx-auto";
        ...
        <div className={`grid gap-6 pt-10 not-prose ${gridCols}`}>
            {packages.map((pkg) => (
                ...
                >
                    ...
                    <div className="text-4xl font-extrabold mb-8 flex items-baseline gap-1">
                        <span className="text-2xl font-bold">₱</span>
                        {pkg.price}
                        <span className="text-sm text-muted-foreground font-normal tracking-normal">{pkg.interval}</span>
                    </div>
