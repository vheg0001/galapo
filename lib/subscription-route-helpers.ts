...
export function pickDisplaySubscription(subscriptions: Subscription[]) {
    ...
    const active = sorted.find((subscription) => getSubscriptionStatus(subscription, EXPIRING_SOON_DAYS) !== "expired" && subscription.status === "active");
    if (active) {
        return active;
    }
        ...
        return {
            ...
            subscription: selectedSubscription
                ? {
                    ...
                    status: getSubscriptionStatus(selectedSubscription, EXPIRING_SOON_DAYS),
                    ...
                    is_expiring_soon:
                        getSubscriptionStatus(selectedSubscription, EXPIRING_SOON_DAYS) === "expiring_soon",
            ...
            top_search_placements: listingPlacements.map((placement) => {
                const linkedPayment = placement.payment_id
                    ...
                    : null;
                const pendingPayment = linkedPayment?.status === "pending";
