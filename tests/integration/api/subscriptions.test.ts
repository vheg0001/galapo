...
import { POST as uploadCombinedProof } from "@/app/api/business/payments/proof/route";
...
describe("Subscription API Integration", () => {
    ...
    describe("POST /api/business/subscriptions (Upgrade)", () => {
        it("creates a pending subscription and returns payment instructions", async () => {
            ...
            // No existing pending subscription
            mockSupabase.single.mockResolvedValueOnce({ data: null, error: null });
            // Mock insert subscription
            mockSupabase.single.mockResolvedValueOnce({ data: { id: "new-sub-1", amount: 599 }, error: null });
            ...
            expect(data.payment_instructions.amount).toBe(599);
    ...
    describe("POST /api/business/payments/proof", () => {
        it("uses configured top search pricing when creating a placement payment", async () => {
            const formData = new FormData();
            formData.append("file", new File(["proof"], "proof.png", { type: "image/png" }));
            formData.append("reference_number", "REF-TS-1");
            formData.append("payment_method", "gcash");
            formData.append("target_id", "placement-1");
            formData.append("payment_type", "top_search");
            ...
            mockSupabase.single
                .mockResolvedValueOnce({
                    data: {
                        id: "placement-1",
                        position: 1,
                        listings: { id: "listing-1", business_name: "Crest Builders Inc." },
                        categories: { name: "Construction" },
                    },
                    error: null,
                })
                .mockResolvedValueOnce({ data: { id: "payment-1" }, error: null });

            mockSupabase.in.mockResolvedValueOnce({
                data: [{ key: "top_search_monthly_price", value: 1299 }],
                error: null,
            });
            mockSupabase.then = (cb: any) => cb({ data: [], error: null });

            const res = asResponse(await uploadCombinedProof(req as any));
            ...
            expect(mockSupabase.insert).toHaveBeenCalledWith(
                expect.objectContaining({
                    ...
                    user_id: "user-123",
                    amount: 1299,
                    description: "Top Search Placement (Pos #1) for Crest Builders Inc. in Construction",
                })
            );
        });
    });
    ...
    describe("POST /api/business/top-search", () => {
        it("creates a pending top search placement and returns payment instructions", async () => {
            ...
            mockSupabase.single
                .mockResolvedValueOnce({ data: { id: "listing-1", owner_id: "user-123" }, error: null })
                .mockResolvedValueOnce({ data: { id: "placement-1", position: 2 }, error: null });
            mockSupabase.maybeSingle
                .mockResolvedValueOnce(undefined)
                .mockResolvedValueOnce({ data: null, error: null })
                .mockResolvedValueOnce({ data: { id: "cat-1", name: "Construction" }, error: null });
            ...
            expect(data.payment).toBeNull();
            expect(data.payment_instructions.amount).toBe(999);
            expect(mockSupabase.insert).toHaveBeenCalledTimes(1);
            expect(mockSupabase.insert).toHaveBeenCalledWith(
                expect.objectContaining({
                    listing_id: "listing-1",
