...
describe("CurrentPlanCard", () => {
    ...
    it("cancels auto-renew after confirmation", async () => {
        (global.fetch as any).mockResolvedValue({
            ok: true,
            json: () => Promise.resolve({ message: "Auto-renewal cancelled successfully" }),
        });
        ...
        expect(screen.getByText("Cancel auto-renew?")).toBeInTheDocument();
        expect(screen.getByText(/Your subscription will remain active until February 1, 2026\./i)).toBeInTheDocument();

        fireEvent.click(screen.getByRole("button", { name: /confirm cancel/i }));

        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith("/api/business/subscriptions/sub-789", {
                method: "PUT",
                headers: {
    ...
    it("cancels a pending subscription request", async () => {
        const pendingItem = {
            ...mockActiveItem,
            subscription: { ...mockActiveItem.subscription!, status: "pending_payment" as any }
        };
        ...
        expect(screen.getByText("Cancel pending subscription?")).toBeInTheDocument();
        expect(screen.getByText("This pending subscription request will be removed.")).toBeInTheDocument();
