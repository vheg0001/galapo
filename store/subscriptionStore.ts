...
export const useSubscriptionStore = create<SubscriptionStoreState>()((set, get) => ({
    ...
    submitPayment: async () => {
        const { currentPaymentId, paymentProof, referenceNumber, paymentMethod, currentFlow } = get();
        if (!currentPaymentId || !paymentProof) {
        ...
        try {
            ...
            const response = await fetch(`/api/business/payments/${currentPaymentId}/proof`, {
                method: "POST",
                body: formData,
            });
